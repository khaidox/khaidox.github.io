...
> **Purpose:** Replace the default Package Name (`me.weishu.kernelsu`) and App Name (`KernelSU`) with customized values (e.g., Package Name: `com.okok.super`, App Name: `Super`) for build customization and package isolation.

---

## 1. Why Customize the Package Name?

Security-sensitive applications and automated testing environments often inspect installed packages for known default package identifiers such as `me.weishu.kernelsu`.

Building KernelSU Manager with a custom Package Name and App Title offers several benefits:
- **Application Isolation:** Avoids package name conflicts and unwanted environment flags during security testing and custom environment builds.
- **Full Code Integrity:** Keeps the core codebase intact while allowing flexible build-time parameter configuration.
- **Cloud Automated Builds:** Leverages GitHub Actions CI/CD to build signed custom APK binaries without local SDK configuration.

---

## 2. How It Works (Based on KernelSU PR #3560)
...
## 3. Step-by-Step Guide: Build Custom APK via GitHub Actions
